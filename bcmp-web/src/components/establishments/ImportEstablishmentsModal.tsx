...
export function ImportEstablishmentsModal({ open, onClose, onSuccess }: Props) {
  ...
  const processFile = async (selectedFile: File) => {
    ...
    setErrorMsg("");
    try {
      const data = await api.establishments.importPreview(selectedFile);
      setPreview(data);
  ...
  const handleExecuteImport = async () => {
    ...
    if (missingRequired) {
      setErrorMsg("Please map all required fields (Business Name and Owner Name).");
      return;
    }
    ...
    try {
      ...
      const res = await api.establishments.importCsv(file, cleanMapping);
      setResult({
  ...
  const getFieldLabel = (field: string) => {
    switch (field) {
      ...
      case "owner_email": return "Owner Email Address";
      case "owner_address": return "Owner Address";
      case "purok": return "Purok/Zone";
      case "street": return "Street Name";
      case "exact_address": return "Exact Address";
      case "registration_type": return "Registration Type (DTI, SEC, etc.)";
      case "registration_number": return "Registration Number";
      case "registration_date": return "Registration Date";
      case "permit_number": return "Permit Number";
      case "permit_expiry": return "Permit Expiration Date";
      ...
      <div className="relative w-full max-w-3xl bg-card rounded-2xl border shadow-xl flex flex-col max-h-[90vh] overflow-hidden scale-in duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">Import Establishments</h2>
              <p className="text-xs text-muted-foreground">Upload an offline establishments Excel or CSV database</p>
            </div>
          </div>
          {step !== "importing" && (
            <button
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <div className="text-center">
                <p className="font-semibold text-foreground text-base">Importing Establishments...</p>
            ...
            <div className="flex flex-col gap-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border bg-green-500/5 text-center flex flex-col gap-1">
                  ...
                  <span className="text-xs font-semibold text-muted-foreground">Establishments Imported</span>
                </div>
                <div className={cn(
