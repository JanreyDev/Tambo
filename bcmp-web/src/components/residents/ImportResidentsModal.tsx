...
export function ImportResidentsModal({ open, onClose, onSuccess }: Props) {
  ...
  const processFile = async (selectedFile: File) => {
    ...
    setErrorMsg("");
    try {
      const data = await api.residents.importPreview(selectedFile);
      setPreview(data);
  ...
  const handleExecuteImport = async () => {
    ...
    if (missingRequired) {
      setErrorMsg("Please map all required fields (First Name and Last Name).");
      return;
    }
    ...
    try {
      ...
      const res = await api.residents.importCsv(file, cleanMapping);
      setResult({
      ...
      <div className="relative w-full max-w-3xl bg-card rounded-2xl border shadow-xl flex flex-col max-h-[90vh] overflow-hidden scale-in duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">Import Residents</h2>
              <p className="text-xs text-muted-foreground">Upload an offline census Excel or CSV database</p>
            </div>
          </div>
          {step !== "importing" && (
            <button
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <div className="text-center">
                <p className="font-semibold text-foreground text-base">Importing Residents...</p>
            ...
            <div className="flex flex-col gap-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border bg-green-500/5 text-center flex flex-col gap-1">
                  ...
                  <span className="text-xs font-semibold text-muted-foreground">Residents Imported</span>
                </div>
                <div className={cn(
