...
export default function ReportsPage() {
  ...
  const handleDownloadReport = (r: ReportTemplate) => {
    addToast(`Downloading last generated report for ${r.name}...`, "success");
    const content = `Mock Report Data for ${r.name}\nGenerated: ${r.last_generated}\nFrequency: ${r.frequency}\nCategory: ${r.category}\n`;
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${r.name.replace(/\s+/g, "_")}_${r.last_generated.replace(/[:\s]/g, "-")}.txt`;
  ...
  const handleDownloadHistoryReport = (r: ReportTemplate, h: { date: string; format: string; size: string; by: string }) => {
    addToast(`Downloading historical report from ${h.date}...`, "success");
    const ext = h.format.toLowerCase() === "pdf" ? "txt" : h.format.toLowerCase() === "excel" ? "xlsx" : "csv";
    const content = `Mock Historical Report: ${r.name}\nDate: ${h.date}\nFormat: ${h.format}\nGenerated by: ${h.by}\n`;
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${r.name.replace(/\s+/g, "_")}_${h.date.replace(/[:\s]/g, "-")}.${ext}`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  const handleDownloadGenerated = () => {
    ...
    const ext = genFormat === "pdf" ? "pdf" : genFormat === "excel" ? "xlsx" : "csv";
    const mime = genFormat === "pdf" ? "application/pdf" : genFormat === "excel" ? "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" : "text/csv";
    const content = `Mock Generated Report: ${generateTarget.name}\nPeriod: ${genPeriod}\nFormat: ${genFormat}\nCharts: ${genCharts ? "Yes" : "No"}\n`;
    const blob = new Blob([content], { type: mime });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `${generateTarget.name.replace(/\s+/g, "_")}.${ext}`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
              ...
              <div className="flex items-center justify-between pt-3 border-t border-border">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button className="p-1.5 rounded hover:bg-muted" title="Download"
                    onClick={(e) => { e.stopPropagation(); handleDownloadReport(r); }}><Download className="h-3.5 w-3.5" /></button>
                  ...
                  <div key={i} className="flex items-center justify-between p-3 rounded-lg glass-subtle">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button className="p-1 rounded hover:bg-muted"
                        onClick={() => handleDownloadHistoryReport(viewTarget, h)}><Download className="h-3.5 w-3.5 text-muted-foreground" /></button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {generateTarget && (
        <Modal open={showSuccess} title="Report Generated" onClose={() => setShowSuccess(false)} size="sm"
          footer={<>
            ...
            <ModalButton variant="primary" onClick={() => { setShowSuccess(false); handleDownloadGenerated(); }}>
