...
export function FCombobox({
  ...
  return (
    <div ref={wrapperRef} className={cn("relative", disabled && "opacity-40 pointer-events-none")}>
      ...
      <div className={cn("flex items-center w-full rounded-xl transition-all duration-200", open ? "glass-input" : "glass-input")}
        style={open ? { borderColor: "var(--accent-primary)", boxShadow: "0 0 0 3px rgba(37, 99, 235, 0.12)" } : undefined}>
        <input
          type="text"
          value={open ? query : value}
          placeholder={value || customPlaceholder || `Type to search or add...`}
          className="flex-1 px-3 py-2.5 text-sm bg-transparent focus:outline-none min-w-0 uppercase placeholder:normal-case"
          disabled={disabled}
        ...
        <div ref={dropdownRef}
          ...
          {matched.length === 0 && !trimmed && (
            <div className="px-3 py-2.5 text-sm text-muted-foreground italic">
              Start typing to add entries. The system learns as you go.
            </div>
          )}
