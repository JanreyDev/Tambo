...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status-card { background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(51, 65, 85, 0.4); border-radius: 16px; padding: 24px; backdrop-filter: blur(16px); margin-bottom: 24px; }
        .status-row { display: flex; align-items: center; justify-content: space-between; padding: 12px 0; }
        .status-row + .status-row { border-top: 1px solid rgba(51, 65, 85, 0.3); }
        .status-label { font-size: 13px; color: #64748b; display: flex; align-items: center; gap: 8px; }
        .status-label svg { width: 16px; height: 16px; opacity: 0.6; }
        .status-value { font-size: 13px; font-weight: 500; color: #cbd5e1; font-family: 'SF Mono', 'Cascadia Code', 'Fira Code', monospace; }
        .status-online { display: inline-flex; align-items: center; gap: 6px; color: #34d399; font-weight: 600; }
        .status-online::before { content: ''; width: 8px; height: 8px; background: #10b981; border-radius: 50%; box-shadow: 0 0 8px rgba(16, 185, 129, 0.5); animation: statusPulse 2s ease-in-out infinite; }

        @keyframes statusPulse { 0%, 100% { box-shadow: 0 0 8px rgba(16, 185, 129, 0.5); } 50% { box-shadow: 0 0 16px rgba(16, 185, 129, 0.3); } }
    ...
    <div class="container">
        ...
        <div class="status-card">
            ...
            <div class="status-row">
                ...
                <span class="status-value">Sanctum + Spatie Permissions</span>
            </div>
            <div class="status-row">
                <span class="status-label">
                ...
                <span class="status-value">PostgreSQL @php try { echo DB::selectOne("SELECT split_part(version(), ' ', 2) AS ver")->ver; } catch (\Throwable $e) { echo 'N/A'; } @endphp</span>
            </div>
        </div>
        ...
        <div class="endpoints">
            ...
            <span class="endpoint"><span class="endpoint-method method-get">GET</span> /dashboard</span>
            <span class="endpoint"><span class="endpoint-method method-get">GET</span> /products</span>
        </div>
